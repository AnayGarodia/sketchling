...
`.follow` only holds the tracked position for its own `[at, at+duration]` window — real `panTo`/`zoomTo` ops are genuine GSAP tweens that hold their end value for any later seek, but `.follow` is a manual overlay evaluated only inside its window, so a seek past `at+duration` falls back to whatever the underlying pan/zoom tweens resolve to there (an earlier `panTo`'s own target, or the default centered pan if none), not a freeze-frame of the last followed position. Give `.follow` a `duration` that covers the whole span you actually want tracked, including any settle/hold tail (particle dust, a spring settling) past the tracked node's own last motion — a `--out` still render defaults to the timeline's true end, so an under-covered follow window shows up as a hard camera snap right at its boundary.

...
Tier 0 lint (off-canvas, degenerate shapes, heavy overlap, off-center composition) runs on every render automatically, for free, before any pixel exists. The overlap check tells containment apart from collision: a small detail almost entirely inside a much bigger shape (an eye on a face, a keyhole on a screen — one shape at least 3x the other's area, barely denting it) reports at `info` level, not `warn` — normal nesting, not two things fighting for the same space. Two comparable-sized shapes overlapping heavily still `warn`. For an intentional overlap the size heuristic doesn't catch (similarly-sized layered rings, e.g.), call `.lintIgnore("overlap")` on either node.

## Primitives

- `sketch.stroke(points, style)` — an open freehand line
- `sketch.loop(points, style)` — a closed freehand shape
- `sketch.blob(cx, cy, radius, style, vertices?)` — an organic, deliberately-not-a-perfect-circle shape. `vertices` (default `10`) controls outline complexity — more reads calmer/rounder, fewer looser/more angular. Below roughly 8-9px radius the outline jitter overwhelms the fill and small blobs stop reading clearly (a "bubble" wants radius ≥9-10 and a light weight, not a tiny radius). The imperfection is baked into the authored points, not just render-time jitter (`looseness`, default `0.3`, perturbs the actual outline coordinates) — a clean-line/ligne-claire register (see `examples/gallery/ligne-claire-look.ts`) wants `looseness: 0` explicitly on every blob, or a window/head reads as hand-wobbled instead of precise.
- `sketch.ellipse(cx, cy, rx, ry, style, vertices?)` — a true, wobble-free ellipse (`rx === ry` for a circle), no seeded jitter in the authored geometry at all (unlike `blob()`, whose wobble floor never reaches zero even at `looseness: 0`). `look: "ink"`'s own render-time sketchiness still applies on top, same as any other stroke. For a sun, a head, a gear hub, a glow ring, a star — anything meant to read as a clean disc. See `examples/gallery/ellipse-shapes.ts`.
...
- `moveTo(x, y)` is a true absolute position: the node's own geometric center ends up at canvas `(x, y)`, regardless of where it currently sits (even after prior `moveBy`/`moveTo` calls). `moveBy(dx, dy)` is relative to wherever the node currently is. Same pairing for rotation: `rotateTo(deg)` is an absolute target, `rotateBy(deg)` turns relative to whatever the node's rotation actually is when the tween starts — including mid-chain through an earlier `rotateTo`/`rotateBy` — so a repeated turn (a crank wound the same amount each time) doesn't need its absolute target computed by hand.
- `.pivotAt(x, y)` (on any node) anchors `rotateTo`/`rotateBy`/`scaleTo` at an absolute canvas point instead of the node's own center — needed whenever something should swing from a point other than its middle (a raised arm pivots at the shoulder, not its own midpoint). **Until `docs/notes/finding-transform-origin.md` is fixed, pass it whenever you rotate or scale anything at all**: with no pivot set, the transform currently happens around the canvas's `(0, 0)` corner rather than the node's own center, so a small `scaleTo` drifts and a large `rotateTo` flings the shape off-canvas. A pivot at the shape's own center is correct either way.
- **Instant state at any point on the timeline**: any animate call with `duration: 0` snaps immediately instead of interpolating — `scope.rotateTo(-10, { at: 4, duration: 0 })` sets the angle outright at t=4. `initial({...})` is the same idea but only for t=0; `duration: 0` is the general form, for a snap at any `at`.
...
- **`moveTo`/`moveAlong` target the node's own geometric bbox center by default, not necessarily whatever point you were thinking of when you built it.** For a vertically lopsided shape (a tall envelope with a small basket hanging off the bottom, a character whose head is much bigger than its feet), the bbox center can sit well away from the "obvious" reference point. Both take an `anchor` option instead of hand-computing the offset: `{ anchor: "top" | "bottom" | "left" | "right" }` targets that edge's own midpoint (`"bottom"` for feet), not a corner — `figure.moveTo(150, groundY, { anchor: "bottom" })` puts the feet exactly on `groundY`. Default (`"center"`, or omitted) is exactly today's behavior. Still worth rendering an early mid-motion frame (`--at`) and checking the silhouette sits where you meant it to, before trusting a longer path.
- `.drawOn()` only reveals `stroke`/`blob`/`loop` nodes and Groups built by `sketch.text()` — a plain `Group` has no single path to trace, so calling `.drawOn()` on one directly is a no-op (validate/lint flags this) — use `.stagger({ effect: "drawOn" })` to draw each child in sequence instead.
...
- The scene's actual rendered length always derives from whatever the last op resolves to — `scene.duration(seconds)` doesn't override that, it declares the length you *intend* so the linter can warn when something is scheduled to end past it (an extrapolated off-screen element, a chain that runs one leg too long) instead of only being noticed as a mysteriously-long rendered video. Only checks ops with an explicit numeric `duration` (not `drawOn`'s auto-pace or `springTo`'s automatic settle window). Optional — nothing changes if you never call it.
- **Two tweens writing the same property on the same node over an overlapping window silently fight** — the later-inserted one wins for as long as both run, no error anywhere. Common real trigger: a reusable idle-sway helper and a scene-specific acting gesture both touching rotation over the same span. The linter catches this statically per property/axis (`x`/`y`, `rotation`, `scaleX`/`scaleY`, `opacity`) — a stride's `moveBy(dx, 0)` overlapping a bob's `moveBy(0, dy)` on the same node is the correct composition and does not warn, since they touch different axes; only two ops on the SAME axis do.
...
Every independent stress test this library has had converged on the same complaint: past a handful of beats, a scene becomes a wall of hand-computed `at` literals (`// ends ~29.5 at x = 1252`) that silently go stale the moment anything upstream is retimed. Two tools fix this:
...
scene.add(sketch.loop([[0, 0], [10, 0], [10, 10], [0, 10]], { smooth: false })) // exhaust puff
  .appear({ at: "liftoff+0.2", duration: 0.4 }); // a moment just after the named point

const flag = sketch.loop([[0, 0], [20, 0], [20, 14], [0, 14]], { smooth: false });
scene.add(flag).drawOn({ at: 0, duration: 0.6 });
flag.fadeTo(0, { at: flag.endAt, duration: 0.3 }); // "right after whatever I just did"
```

- **`scene.label(name, seconds)`** names a point on the scene's shared timeline. Reference it later as an `at` value: the plain name (`"liftoff"`), or with an offset (`"liftoff+0.4"` / `"liftoff-0.2"`). Every fluent animate call, and `scene.camera()`'s own `panTo`/`panBy`/`zoomTo`/`follow`, accept this anywhere a plain number works. Declare a label in any order relative to what references it — resolution happens once, at render time, not as each call is made. An unknown label or malformed reference throws immediately with the bad name in the message, instead of silently resolving to `NaN`. For referencing one point in time from several different nodes/ops — the caption-tracks-the-camera-pan pattern, several moving parts keying off one moment — where re-deriving the same literal in five places is exactly how they drift out of sync.
- **`node.endAt`** is the end time (seconds) of the most recently added animation on that node — `at + duration`, using the same default duration each op kind actually renders with when none is given, and a length-based estimate for an undecorated `drawOn`. For continuing the SAME node's own sequence without hand-summing durations: `node.moveTo(...); node.rotateTo(..., { at: node.endAt, ... })` instead of tracking a running total by hand. Throws if the last op's own `at` is still an unresolved label reference — nothing concrete to add a duration to before that resolves. Empty animations list reads as 0.
- Both resolve to plain numbers before the renderer/linter/agent-validation ever see them — a scene that never uses either behaves exactly as it always has.
...
- `scene.camera()` returns a controller with `.panTo(x, y, opts)` (centers the viewport on a world-space point — an ABSOLUTE point, not a delta: a "barely-perceptible drift" meant as a 6px/-2px nudge has to target `panTo(326, 198, ...)` on a 640×400 canvas whose default center is `(320, 200)`, not `panTo(6, -2, ...)`, or the camera races toward scene-space `(6,-2)` and drags the whole frame off by the end of the tween), `.panBy(dx, dy, opts)` (relative to wherever the camera actually is when the tween starts — including mid-follow or after an earlier pan — for when the delta is genuinely what you mean instead of computing an absolute target by hand), `.zoomTo(scale, opts)` (1 = authored size, >1 closer in), and `.follow(node, opts)` (keeps the viewport centered on a node for the duration, tracking wherever its own `moveTo`/`moveBy`/`moveAlong` actually takes it — not a fixed point recorded once). `.follow` keeps tracking past its own `at+duration` — `duration` is when it becomes eligible to be superseded, not a hard cutoff. It stays in control until a later `panTo`/`zoomTo`/`follow` actually takes over; with nothing scheduled after it, it holds the node's live position for the rest of the scene, including any settle/hold tail past the node's own last motion. No need to pad `duration` defensively — there's no snap. Separately, keep the camera's own framing inside the world's bounds on every axis (`center ± viewport/2` within `[0, WORLD_W]`/`[0, WORLD_H]`) — a viewport extended past a world edge (a `follow` target ending up close enough to one, e.g.) shows bare `scene.background` where the depth-1 content has run out, reading as a stray pale rectangle (the backdrop sits at depth 0 and never pans, so it's just the still background showing through). The Tier 0 linter now catches this directly on any `panTo` that frames past the world's edge, but it's still worth leaving at least half the viewport's width/height of margin at every edge (see `quiet-ride.ts`).
...
- **Give the chain real reach headroom.** `len1 + len2` should comfortably exceed the distance it actually needs to cover — a chain at or near full extension gets its target silently clamped onto the reachable radius the moment it's asked to reach slightly further, which distorts the effective foot position (and, in a walk cycle, breaks the planted-foot-doesn't-slide guarantee below). Some bend at rest reads as more natural anyway. The linter checks this statically (root and every `ikTo` target are both known at build time) and names the actual overage in pixels, instead of only being visible once rendered as a rigid "plank" limb.
...
- `sketch.particles(x, y, style, {count?, angle?, spread?, speedMin?, speedMax?, gravity?, lifetime?, duration?, at?, sizeMin?, sizeMax?, fade?})` launches `count` (default 24) small dots from `(x, y)` within a cone (`angle` ± `spread`/2 degrees, screen convention 0 = +x/right, 90 = +y/down — default `angle: -90` is straight up) under constant `gravity` (default 220 px/sec², pulls toward +y). `duration` (default 0) spreads emission across a window instead of firing all at once; `lifetime` (default 1.2s) is how long each particle stays visible, `fade` (default true) ramping its opacity in then out over that span. `shape` (default `"dot"`) can be `"streak"` instead — a short line along each particle's own instantaneous velocity (recomputed every seek as gravity bends it) — for rain/sparks/debris, where a fast-moving round dot still reads as a dot, not motion. `moveTo: {x, y, duration, ease?}` moves the emitter's own spawn point in a straight eased line from its constructor `(x, y)` to `{x, y}`, starting at the SAME `at` the emitter itself starts at — for particles emitted from something moving (dust trailing a boat, sparks off a rising rocket) without re-authoring that motion a second time on the particles node. Resolved once per particle at ITS OWN spawn time (closed-form, since both endpoints and the duration are fixed), so an early-launched particle trails behind where the source was then, not its current position.
...
`look` controls the GEOMETRY treatment (how paths/fills/timing are computed); `texture` is an OPTIONAL whole-frame post-process layered on top, freely combinable with any of `"ink"`/`"flat"`/`"clay"` — a watercolor wash over ink's own sketchy jitter, or grain over flat's crisp precision, are both real and equally valid. This wasn't always two fields: an earlier revision folded watercolor/grain/pixel into `look` itself, forcing them to share flat's crisp geometry — workable until grain (aged paper) needed to pair with ink's hachure fills for an old-book/engraving register, and crisp-only broke that combination outright. Split into two independent axes instead of guessing at more special-cased look values.

**`look` values:**
- `"ink"` (default) is the hand-drawn look everything above assumes — sketchy jitter, line boil, a visible pen tip tracing `drawOn`. `"flat"` renders the *identical* authored scene — same geometry, same physics, same timing — crisp and precise instead: no jitter, no boil, solid fills instead of hachure/cross-hatch, no pen tip.
...
- `"grain"`: a different whole-frame SVG filter aimed at a different target — fractal noise converted to a pure-black layer whose alpha (not color) varies with noise brightness, blended over the source via `feBlend "overlay"` (darkens shadows/lightens highlights slightly, the way real grain modulates an image, not a flat semi-transparent layer sitting on top) — fine aged-paper/film-grain texture instead of wet-media bleed.
- `"pixel"`: a raster post-process on every captured frame instead — downsample to a low-res grid, then nearest-neighbor upscale back to size, so every cell lands as one flat-colored block. Applied outside the DOM, in the CLI, on the captured PNG bytes (via `ffmpeg`), not as an SVG filter like watercolor/grain — the only texture that needs `ffmpeg` on `PATH` outside of `--video`. Because it lives in the CLI's capture step operating on the final composited frame, it stays scene-only (a `Film` entry using it doesn't get pixelated — there's no way to selectively pixelate one entry's screen region after everything's already composited into one canvas, unlike watercolor/grain, which are SVG filters scoped to each entry's own content and DO apply correctly inside a `Film`) and doesn't show under `--serve` (which skips the capture step entirely).
- Nothing about how you author a scene changes based on `look`/`texture` — every primitive, animation, and this whole reference applies the same either way. Both are rendering decisions, not authoring ones.
...
Each scene keeps its own width/height/background and animates completely independently; `Film` just scales/centers each one into its own canvas (against that scene's own output frame — `viewportWidth`/`viewportHeight`, not its world `width`/`height`, so a `scene.camera()` scene inside a wider world composes at the right size instead of shrinking; each scene is also clipped to that frame so a camera pan can't spill content past its intended bounds) and sequences them, letterboxing whatever doesn't match the film's aspect ratio. `transition` is `"cut"` (instant, default) or `"fade"` (crossfade over `transitionDuration` seconds). `hold` is how long a scene sits on its settled frame before the next one takes over. Lint runs per-scene, prefixed with the scene's index.
...
- Tier 0's containment note (`info` level) on shapes nested on purpose (an eye inside a head) — expected. A remaining `warn`-level overlap between comparable-sized shapes is worth a look, or `.lintIgnore("overlap")` if it's intentional.

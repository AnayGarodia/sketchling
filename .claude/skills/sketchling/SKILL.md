...
- `"pixel"`: `"flat"`'s crisp geometry again, plus a raster post-process on every captured frame — downsample to a low-res grid, then nearest-neighbor upscale back to size, so every cell lands as one flat-colored block. Applied outside the DOM, in the CLI, on the captured PNG bytes (via `ffmpeg`), not as an SVG filter like watercolor's — the only look that needs `ffmpeg` on `PATH` outside of `--video`.

...
Status: **reported, not fixed.** Found while building the browser playground (`site/`), where
it is the first thing a newcomer hits. Deliberately left alone because the fix changes the
rendered output of most of `examples/` — including the films in the README — so it is the
library author's call, not a playground change.

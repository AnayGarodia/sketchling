...
Seventeen more scenes in [`examples/gallery/`](examples/gallery/), proving out IK rigging, the procedural walk generator and auto-rig, secondary-motion springs and connectors, particles, audio, gradient shading, and every `look`/`texture` combination — each one still an animation (`docs/gallery-*.mp4`), not just a still.
...
Five longer scenes in [`examples/showcase/`](examples/showcase/), composing the capabilities above together rather than demonstrating one at a time.

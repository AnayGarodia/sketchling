...
Fifteen more scenes in [`examples/gallery/`](examples/gallery/), proving out IK rigging, the procedural walk generator and auto-rig, secondary-motion springs and connectors, particles, audio, gradient shading, and all eight render `look`s — each one still an animation (`docs/gallery-*.mp4`), not just a still.
...
Four longer scenes in [`examples/showcase/`](examples/showcase/), composing the capabilities above together rather than demonstrating one at a time.
...
`fill.color` takes a gradient too — the same `{ stops: [{offset, color}], direction? }` shape `scene.background` already takes — for a real per-shape SVG gradient instead of a flat color: volumetric shading on one form (a landform lit from above and falling into shadow, a body with a shaded underside) rather than a uniform fill. Only renders as an actual gradient under `fill.style: "solid"` (or a look that forces solid — `"flat"`/`"watercolor"`/`"pixel"`/`"clay"`); the procedural line fills have no continuous area to gradient across, so a gradient color there degrades to its first stop instead. See `examples/gallery/gradient-shading.ts`.
...
One authored scene — the same geometry, physics, and timing — can render through seven different visual treatments, because "what happens" and "how it looks" are separate concerns here. `look` on `sketch.scene(...)` picks the treatment:
...
- **`"pixel"`** — `"flat"`'s crisp geometry again, plus a raster post-process applied to every captured frame after the browser screenshot: downsample to a low-resolution grid, then scale back up with nearest-neighbor (no smoothing), so every cell lands as one flat-colored block. This happens outside the DOM entirely (in the CLI, on the captured PNG bytes, via `ffmpeg`), not as an SVG filter like watercolor's — the only look that needs `ffmpeg` on `PATH` outside of `--video`. Two consequences of living in the CLI's capture step rather than the renderer: it's scene-only (a `Film` entry with `look: "pixel"` renders at full resolution, uncropped by this post-process — the pixelation isn't wired into `Film` compositing), and `--serve` won't show it either, since that mode skips the CLI's capture step entirely and opens the raw page in a browser.

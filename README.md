...
Sketchling is designed for the full agent loop: author a scene, render evidence, inspect
machine-readable state, then repair from concrete diagnostics. The CLI never asks an agent
to infer a renderer limitation from a missing pixel.

```sh
# Contract and structural checks. JSON is stable enough to feed straight back to an agent.
sketchling validate scene.ts --json

# Node list, bounds, animation kinds, estimated duration, lint, and capability warnings.
sketchling inspect scene.ts

# A rendered artifact plus its exact browser-resolved duration and findings.
sketchling render scene.ts --out preview.png --json

# Six evenly-spaced frames in one PNG for fast visual review.
sketchling contact-sheet scene.ts --out review.png
```

Give important elements an author-stable name so a diagnostic can point to intent rather
than an ephemeral auto-id:

```ts
scene.add(sketch.blob(240, 180, 52, style).named("hero-moon"));
```

`validate` reports unsupported backend behavior before render. For example, it warns when a
2D stroke is authored into a `"toon3d"` scene, or when a texture is requested for the 3D
renderer. This is deliberate: agents need explicit capability boundaries, not silent drops.
...
Nineteen more scenes in [`examples/gallery/`](examples/gallery/), proving out IK rigging, the procedural walk generator and auto-rig, secondary-motion springs and connectors, particles, audio, gradient shading, and every `look`/`texture` combination — each one still an animation (`docs/gallery-*.mp4`), not just a still.

...
Fourteen more scenes in [`examples/gallery/`](examples/gallery/), proving out IK rigging, the procedural walk generator and auto-rig, secondary-motion springs and connectors, particles, audio, gradient shading, and all seven render `look`s — each one still an animation (`docs/gallery-*.mp4`), not just a still.

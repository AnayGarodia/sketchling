import type { NodeStyle, Weight } from "../core/types.js";
...
/** Maps sketchling's design vocabulary (weight/looseness/energy) onto rough.js draw options. */
export function roughOptionsFor(style: NodeStyle, seed: number, closed: boolean): Record<string, unknown> {
  ...
  const opts: Record<string, unknown> = {
    ...
    roughness: (0.6 + looseness * 2.2) * energyMult,
    bowing: (0.5 + looseness * 2) * energyMult,
    seed,
    preserveVertices: false,
  };
  if (closed && style.fill) {
    opts.fill = style.fill.color;
    opts.fillStyle = style.fill.style ?? "hachure";

...
- `.pivotAt(x, y)` (on any node) anchors `rotateTo`/`rotateBy`/`scaleTo` at an absolute canvas point instead of the node's own center — needed whenever something should swing from a point other than its middle (a raised arm pivots at the shoulder, not its own midpoint).
